...
static void *worker_thread(void *arg) {
    ...
    if (alive) {
        ...
        for (size_t i=0;i<sizeof(ports_to_check)/sizeof(int);i++){
            ...
            if (scan_port_connect(ip, p, ctx->timeout_ms)) {
...
int main(int argc, char **argv) {
    ...
    sem_init(&ctx->sem, 0, ctx->max_threads);
    ...
    char netinfo[256];
    snprintf(netinfo, sizeof(netinfo), "Network: %s  Range: %s - %s", ctx->network, inet_ntop(AF_INET, &saddr, (char[INET_ADDRSTRLEN]){0}, 0)?inet_ntop(AF_INET, &saddr, (char[INET_ADDRSTRLEN]){0}, 0):"", inet_ntop(AF_INET, &eaddr, (char[INET_ADDRSTRLEN]){0}, 0)?inet_ntop(AF_INET, &eaddr, (char[INET_ADDRSTRLEN]){0}, 0):"");
